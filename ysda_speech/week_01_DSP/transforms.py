...
class Windowing:
    ...
    def __call__(self, waveform):
        waveform = np.pad(
            waveform, (self.window_size // 2, self.window_size // 2)
        )

        windows = np.array(
            [
                waveform[i : i + self.window_size]
                for i in range(
                    0, len(waveform) - self.window_size + 1, self.hop_length
                )
            ]
        )
        return windows
...
class Hann:
    def __init__(self, window_size=1024):
        self.hann = np.expand_dims(
            scipy.signal.windows.hann(window_size, sym=False), axis=0
        )
